...
def deepforest_crowns_gdf(
    ...
    """Return individual tree crown polygons from DeepForest detections.

    Unlike deepforest_mask(), this preserves per-crown geometry and computes
    height individually per crown, preventing the sqrt(N) inflation that occurs
    when merged crowns are treated as one component.

    Returns a GeoDataFrame (EPSG:25832) with the same schema as vectorize_trees():
      tree_id, geometry (Polygon), height_m, crown_radius_m, crown_area_m2,
      vegetation_model.
    """
    import math
    ...
    west_m, south_m = to_utm.transform(bbox["west"], bbox["south"])
    east_m, north_m = to_utm.transform(bbox["east"], bbox["north"])
    pixel_size_m = ((east_m - west_m) / W + (north_m - south_m) / H) / 2.0
    transform = from_bounds(west_m, south_m, east_m, north_m, W, H)

    predictions = model.predict_tile(
        image=img, patch_size=patch_size,
        patch_overlap=patch_overlap, iou_threshold=iou_threshold,
    )

    records = []
    if predictions is not None and len(predictions) > 0:
        for tree_id, (_, row) in enumerate(
            predictions[predictions["score"] >= score_threshold].iterrows(), start=1
        ):
            cx_px = (row["xmin"] + row["xmax"]) / 2
            cy_px = (row["ymin"] + row["ymax"]) / 2
            crown_r_px = ((row["xmax"] - row["xmin"]) + (row["ymax"] - row["ymin"])) / 4
            x_utm, y_utm = transform * (cx_px, cy_px)
            crown_r_m = crown_r_px * pixel_size_m
            height_m = 2.0 * crown_r_m * 0.7
            records.append({
                "tree_id": tree_id,
                "geometry": Point(x_utm, y_utm).buffer(crown_r_m, resolution=16),
                "height_m": round(height_m, 3),
                "crown_radius_m": round(crown_r_m, 3),
                "crown_area_m2": round(math.pi * crown_r_m ** 2, 2),
                "vegetation_model": "deepforest",
            })
